...
const NavBar = (props: Props) => {
  ...
  return (
    <header className="sticky top-0 z-30 w-full backdrop-blur-md bg-slate-900/80 border-b border-slate-800/80 px-6 py-3 transition-all">
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-3 w-1/3">
          <div className="relative w-full max-w-md">
            ...
            <input
              type="search"
              className="block w-full pl-10 pr-4 py-2 text-sm rounded-xl bg-slate-800/90 border border-slate-700/60 text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
              placeholder="Search inventory, SKU, or ask AI agent..."
            />
          </div>
        </div>

        {/* Center Badge / Title for Mobile */}
        ...
        <div className="flex items-center space-x-4">
          {!isLoggedIn() ? (
            <div className="flex items-center space-x-3">
              ...
              <a
                href="/register"
                className="text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 px-4 py-2 rounded-xl shadow-lg shadow-indigo-500/20 transition-all hover:scale-105 active:scale-95">
                Sign Up
              </a>
            </div>
          ) : (
