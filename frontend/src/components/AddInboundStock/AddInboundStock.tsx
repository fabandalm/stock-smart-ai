...
import { Product, Supplier } from "../../helpers/declarations";
import { AllProducts } from "../../services/ProductService";
import { AllSuppliers } from "../../services/SupplierService";

interface ModelProps {
  isOpen: boolean;
  onClose: (data?: AddInboundStockDTO) => void; // A function type that takes no arguments and returns void
}
export interface AddInboundStockDTO {
  productId: number;
  supplierId: number;
  quantity: number;
  date: Date;
...
const AddInboundStock = (props: ModelProps) => {
  const [FormsValues, setFormsValues] = useState<AddInboundStockDTO>({
    date: new Date(),
    productId: 0,
    supplierId: 0,
    quantity: 0,
          ...
          <form
            ...
            <div className="grid gap-4 mb-4 grid-cols-12">
              ...
              <div className="col-span-6 sm:col-span-6">
                ...
                <select
                  id="product"
                  value={FormsValues?.productId || ""}
                  onChange={(e) => {
                    setFormsValues((prev) => ({
                      ...prev,
                      productId: Number(e.target.value), // Convert the value to a number
                    }));
                  }}
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
...
export default AddInboundStock;
