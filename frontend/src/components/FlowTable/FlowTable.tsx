    ...
    <div>
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {props.iOStock.map((operation) => (
              <tr
                key={`${operation.id}_${operation.type}`}
                className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                <th
                  scope="row"
                  className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <span className={` flex justify-center items-center py-2 px-4 rounded-3xl text-white ${
                ...
                <td className="px-6 py-4">{operation.product}</td>
                <td className="px-6 py-4">{operation.facilitator}</td>
